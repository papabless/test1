    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=1115&limit=11&order_by=desc", function(json){
                    $('#game_allbets_logs').html("");
                    if(json.length > 0){
                        $.each(json.reverse(), function(index){
                            var table = document.getElementById("game_allbets_logs");
                            
                            var row = table.insertRow(0);
                            row.id = "bet_"+json[index].id;
                            ...
                            var win = parseFloat(json[index].profit) >= 0;
                            
                            cell1.innerHTML = "<a href=\"https://www.moneypot.com/bets/"+json[index].id+"\" target=\"blank\">"+json[index].id+"</a>";
                            cell1.className = (win?"win":"lose");

                            cell2.innerHTML = json[index].uname;
                            cell3.innerHTML = (json[index].wager/100).formatMoney(2, '.', ',')+" Bits";
                            
                            cell4.innerHTML = (win?"+":"")+(json[index].profit/100).formatMoney(2, '.', ',');
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=1115&wager=50000&limit=11&order_by=desc", function(json){
