    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=584&wager=1000000", function(json){
                    if(json.length > 0){
                        //$('#highRoller_log').css('display', "inline-block");
                        console.log("HigRoller list test: ",json);
                        /*$.each(json, function(index){
                            if($(this).profit <= 0){
                                $('#highRoller_logs').html("<div class=\"log_item\" style=\"border-left: 4px solid #E74C3C;\">"+$(this).uname+" <br> Waggered: <span style=\"color: #E74C3C\">"+(parseFloat($(this).wager).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+$(this).id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
                            }else{
                                $('#highRoller_logs').html("<div class=\"log_item\" style=\"border-left: 4px solid #8D4;\">"+$(this).uname+" <br> Waggered: <span style=\"color: #8D4\">"+(parseFloat($(this).wager).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+$(this).id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
                            }
                        });*/
                    }
                });
