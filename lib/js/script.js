...
$(function(){
    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            $('.safeLink').click(function(){
                var currentSafeLink = $(this).text();
                $('#unknownLink').text(currentSafeLink);
                $('.safeLinkDoRedirectButton').attr("href", currentSafeLink);
                $('#safeLinkForm').modal('setting', 'closable', false).modal('show');

                $('#safeLinkCancelRedirectButton').click(function(){
    ...
    socket.on('new_bet', function(payload) {
        ...
        if((payload.wager/100)>=5000){
            if(payload.profit <= 0){
                $('#highRoller_logs').html("<div class=\"hr_log_item\" style=\"border-left: 4px solid #E74C3C;\"><b><a class=\"hr_log_item_username\" href=\"https://www.moneypot.com/users/"+payload.uname+"\">"+payload.uname+"</a></b>: <span style=\"color: #E74C3C\">"+(parseFloat(payload.wager/100).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+payload.id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
            }else{
                $('#highRoller_logs').html("<div class=\"hr_log_item\" style=\"border-left: 4px solid #8D4;\"><b><a class=\"hr_log_item_username\" href=\"https://www.moneypot.com/users/"+payload.uname+"\">"+payload.uname+"</a></b>: <span style=\"color: #8D4\">"+(parseFloat(payload.wager/100).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+payload.id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
            }
...
function addNewChatMessage(data){
    ...
    var modStyle = ((rank=="OWNER")?"style='color:#E74C3C;":(rank=="ADMIN"?"style='color:#EC87C0;":(rank=="MOD"?"style='color:#2ECC71;":"style='")));
    if(isInHighRollersList(username)) modStyle += "background:url(\"lib/css/img/sparkle.gif\");";
    modStyle += "'";
    chatMonitor.innerHTML += "<span class=\"chatMessage "+(notify?"notify":"")+"\" "+servStyle+"><small>"+date.hours+":"+date.mins+"</small> <b "+modStyle+">"+username+"</b>: "+urlify(message)+"<br></span>";
    chatMonitor.scrollTop = chatMonitor.scrollHeight;
...
$('#tipButton').click(function(){
    $('#tippingForm').modal('setting', 'closable', false).modal('show');
});

$('#sendTipButton').click(function(){
    $('#sendTipButton').addClass('loading');
    $('#sendTipButton').attr("disabled", true);

    var username = $('#tipping_username_input').val();
    var amount = $('#tipping_amount_input').val();
    var confirmAlert = confirm("Are you sure to tip "+amount+" Bits to "+username+" ?");
    if(confirmAlert == true && amount == parseInt(amount)){
        $.ajax({
            type: "POST",
            ...
            if(data.id){
                addNewChatMessage({
                    ...
                    text: "Sent "+amount+" Bits to "+username
                });
            ...
            $('#tippingForm').modal('hide');
            $('#sendTipButton').removeClass('loading');
            $('#sendTipButton').attr("disabled", false);
            $('#tipping_username_input').val("");
            $('#tipping_amount_input').val("");
        });
    }
});
...
function urlify(text){
    ...
    return text.replace(urlRegex, function(url){
        return '<a class="safeLink">' + url + '</a>';
    });
}
function isInHighRollersList(username){
    var nameFound = false;
    $('.hr_log_item_username').each(function(){
        if($(this).text() == username) nameFound = true;
    });
    return nameFound;
}
