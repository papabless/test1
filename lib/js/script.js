...
function startNewGame(){
    ...
    $(".tile").each(function(ndx){
        $(this).click(function(){
            ...
            // 0 to 4294967296 
            var rageLose = Math.round(4294967296/((4294967296/25)*(game.bombs+game.tilesClicked)));
            ...
            $.ajax({
                ...
                if(data.next_hash){
                    ...
                    if(data.outcome < rageLose){
                        ...
                        self.html("<span class=\"tile_val\">+"+(parseFloat(parseFloat(game.next).formatMoney(2,'.',',')))+"</span>");
