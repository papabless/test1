    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=1115&wager=9900000&limit=11&order_by=desc", function(json){
...
function startNewGame(){
    if(!connected){
        $('#theDimmer').html("Error: Not connected!<br><a href=\"https://www.moneypot.com/oauth/authorize?app_id=1115&response_type=token&state=Meh&redirect_uri=http://turtledaddykim.github.io/bitsweeper\">Click here to login</a>");
...
function tipUser(username, amount){
    $.ajax({
        ...
        if(data.id){
            ...
            user_balance = ((user_balance-(amount*100))/100);
            $('#balance').text((user_balance).formatMoney(2,'.',','));
        }else{
            addNewChatMessage({
                created_at: (new Date()).getTime().toISOString(),
