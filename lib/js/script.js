                    ...
                    if(json.length > 0){
                        ...
                        $.each(json, function(index){
                            if(json[index].profit <= 0){
                                ...
                                $('#highRoller_logs').html("<div class=\"log_item\" style=\"border-left: 4px solid #8D4;\">"+json[index].uname+" <br> Waggered: <span style=\"color: #8D4\">"+(parseFloat(json[index].wager/100).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+json[index].id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
                            }
                        });
                    }
                });
