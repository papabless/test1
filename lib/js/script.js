    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=584&wager=1000000&limit=10", function(json){
                    if(json.length > 0){
                        $('#highRoller_log').css('display', "block");
                        $.each(json, function(index){
                            if(json[index].profit <= 0){
                                $('#highRoller_logs').html("<div class=\"log_item\" style=\"border-left: 4px solid #E74C3C;\"><b><a href=\"https://www.moneypot.com/users/"+json[index].uname+"\">"+json[index].uname+"</a></b>: <span style=\"color: #E74C3C\">"+(parseFloat(json[index].wager/100).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+json[index].id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
                            }else{
                                $('#highRoller_logs').html("<div class=\"log_item\" style=\"border-left: 4px solid #8D4;\"><b><a href=\"https://www.moneypot.com/users/"+json[index].uname+"\">"+json[index].uname+"</a></b>: <span style=\"color: #8D4\">"+(parseFloat(json[index].wager/100).formatMoney(2,'.',','))+"</span> Bits! <div class=\"info\"><a href=\"https://www.moneypot.com/bets/"+json[index].id+"\" target=\"_blank\"><i class=\"info circle icon\"></i></a></div></div>"+$('#highRoller_logs').html());
                            }
                        });
                    }
                });
